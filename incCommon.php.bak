<?php
	...
	*/
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			'submitlog' => "`submitlog`.`submid` as 'submid', `submitlog`.`cstatus` as 'cstatus', DATE_FORMAT(`submitlog`.`logtime`, '%c/%e/%Y %l:%i%p') as 'logtime', `submitlog`.`ipaddr` as 'ipaddr', `submitlog`.`company` as 'company', `submitlog`.`country` as 'country', `submitlog`.`website` as 'website', `submitlog`.`contactname` as 'contactname', `submitlog`.`email` as 'email', `submitlog`.`title` as 'title', `submitlog`.`version` as 'version', if(`submitlog`.`pdate`,date_format(`submitlog`.`pdate`,'%m/%d/%Y'),'') as 'pdate', `submitlog`.`cost` as 'cost', `submitlog`.`ptype` as 'ptype', `submitlog`.`install` as 'install', `submitlog`.`os` as 'os', `submitlog`.`languages` as 'languages', `submitlog`.`changeinfo` as 'changeinfo', `submitlog`.`category` as 'category', `submitlog`.`requirements` as 'requirements', `submitlog`.`ksize` as 'ksize', `submitlog`.`keywords` as 'keywords', `submitlog`.`description` as 'description', `submitlog`.`descrlarge` as 'descrlarge', `submitlog`.`homepage` as 'homepage', `submitlog`.`screenshot` as 'screenshot', `submitlog`.`icon` as 'icon', `submitlog`.`padfile` as 'padfile', `submitlog`.`download` as 'download', `submitlog`.`aspnumber` as 'aspnumber', `submitlog`.`backlink` as 'backlink', `submitlog`.`affiliate` as 'affiliate', `submitlog`.`affiliateid` as 'affiliateid'",
			'paddata' => "`paddata`.`progid` as 'progid', `paddata`.`company` as 'company', `paddata`.`country` as 'country', `paddata`.`website` as 'website', `paddata`.`contactname` as 'contactname', `paddata`.`email` as 'email', `paddata`.`title` as 'title', `paddata`.`version` as 'version', if(`paddata`.`pdate`,date_format(`paddata`.`pdate`,'%m/%d/%Y'),'') as 'pdate', `paddata`.`cost` as 'cost', `paddata`.`ptype` as 'ptype', `paddata`.`status` as 'status', `paddata`.`install` as 'install', `paddata`.`os` as 'os', `paddata`.`languages` as 'languages', `paddata`.`changeinfo` as 'changeinfo', `paddata`.`category` as 'category', `paddata`.`requirements` as 'requirements', `paddata`.`ksize` as 'ksize', `paddata`.`keywords` as 'keywords', `paddata`.`description` as 'description', `paddata`.`descrlarge` as 'descrlarge', `paddata`.`homepage` as 'homepage', `paddata`.`screenshot` as 'screenshot', `paddata`.`icon` as 'icon', `paddata`.`padfile` as 'padfile', `paddata`.`download` as 'download', `paddata`.`dlcount` as 'dlcount', `paddata`.`dlipaddr` as 'dlipaddr', `paddata`.`aspnumber` as 'aspnumber', `paddata`.`affiliate` as 'affiliate', `paddata`.`affiliateid` as 'affiliateid', `paddata`.`siterating` as 'siterating', `paddata`.`clean` as 'clean', `paddata`.`paderrcount` as 'paderrcount'",
			'customurls' => "`customurls`.`customid` as 'customid', `customurls`.`progid` as 'progid', `customurls`.`customurl` as 'customurl'",
			'bannerlist' => "`bannerlist`.`bannerid` as 'bannerid', `bannerlist`.`imgurl` as 'imgurl', `bannerlist`.`linkurl` as 'linkurl', `bannerlist`.`listdisp` as 'listdisp', `bannerlist`.`dldisp` as 'dldisp', `bannerlist`.`authdisp` as 'authdisp'",
			'blacklist' => "`blacklist`.`blackid` as 'blackid', `blacklist`.`bldomain` as 'bldomain'",
			'usrreviews' => "`usrreviews`.`revid` as 'revid', `usrreviews`.`progid` as 'progid', `usrreviews`.`useripaddr` as 'useripaddr', `usrreviews`.`revtime` as 'revtime', `usrreviews`.`username` as 'username', `usrreviews`.`userrating` as 'userrating', `usrreviews`.`usercomment` as 'usercomment'"
		);
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('SWLDBA_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('SWLDBA_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['SWLDBA_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['SWLDBA_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> SWLDBA</a>
			</div>
				...
				<div class="col-xs-12 <?php echo $link['grid_column_classes']; ?>">
					<div class="panel <?php echo $link['panel_classes']; ?>">
						<div class="panel-body">
							<a class="btn btn-block btn-lg <?php echo $link['link_classes']; ?>" target="($link['target'])" title="<?php echo preg_replace("/&amp;(#[0-9]+|[a-z]+);/i", "&$1;", htmlspecialchars(strip_tags($link['description']))); ?>" href="<?php echo $link['url']; ?>"><?php echo ($link['icon'] ? '<img src="' . $link['icon'] . '">' : ''); ?><strong><?php echo $link['title']; ?></strong></a>
